...
const DESIGN_CATEGORIES = ['UX/UI', 'グラフィック', 'タイポグラフィ', 'ブランディング', 'Web', '配色', '写真・レイアウト', 'その他'];
const DESIGN_CATEGORY_COLORS = {
    'UX/UI':       { bg: '#e3f2fd', text: '#1565c0' },
    'グラフィック':  { bg: '#fce4ec', text: '#c62828' },
    'タイポグラフィ': { bg: '#f3e5f5', text: '#6a1b9a' },
    'ブランディング': { bg: '#fff3e0', text: '#e65100' },
    'Web':          { bg: '#e8f5e9', text: '#2e7d32' },
    '配色':          { bg: '#fffde7', text: '#f57f17' },
    '写真・レイアウト': { bg: '#e0f7fa', text: '#00695c' },
...
function renderData(allData) {
    ...
    designBooksData = allData.designBooks || [];
...
function renderDesignBooks() {
    const grid = document.getElementById('designGrid');
    if (!grid) return;
    const searchVal = (document.getElementById('searchBoxDesign')?.value || '').toLowerCase();
    const catVal = document.getElementById('designCategoryFilter')?.value || '';
    let filtered = designBooksData.filter(item => {
        const text = (item.title + (item.author || '') + (item.memo || '')).toLowerCase();
        return text.includes(searchVal) && (catVal === '' || item.category === catVal);
    });
    if (filtered.length === 0) {
        grid.innerHTML = '<p style="color:white; text-align:center; grid-column:1/-1;">デザイン本がまだ登録されていません。右下の「＋」から追加しましょう！</p>';
        return;
    }
    grid.innerHTML = filtered.map(item => {
...
function confirmDeleteDesignBook(title, event) {
    event.stopPropagation();
    if (!confirm(`「${title}」をデザイン本棚から削除しますか？`)) return;
    sendToGAS({ action: 'deleteDesignBook', title }, null, '');
}

// Wishlist → デザイン本棚
let designFromWishPayload = {};

function openDesignFromWishModal(title, creator, coverUrl, externalId) {
    designFromWishPayload = { title, creator, coverUrl, externalId };
    document.getElementById('designFromWishTitle').textContent = title;
    document.getElementById('designFromWishCategory').value = 'グラフィック';
    document.getElementById('designFromWishMemo').value = '';
    document.getElementById('designFromWishModal').classList.add('active');
}

// デザイン本 直接追加
let selectedDesignCandidate = null;

function openAddDesignModal() {
    selectedDesignCandidate = null;
    document.getElementById('addDesignSearchInput').value = '';
    document.getElementById('addDesignCreatorInput').value = '';
    document.getElementById('addDesignSearchStatus').style.display = 'none';
    document.getElementById('addDesignCandidates').innerHTML = '';
    document.getElementById('addDesignManualArea').style.display = 'none';
    document.getElementById('addDesignMemo').value = '';
    document.getElementById('addDesignCoverUrl').value = '';
    document.getElementById('addDesignStep1').style.display = 'block';
    document.getElementById('addDesignStep2').style.display = 'none';
    document.getElementById('addDesignModal').classList.add('active');
    const input = document.getElementById('addDesignSearchInput');
    const creatorInput = document.getElementById('addDesignCreatorInput');
    const doSearch = debounce(() => executeAddDesignSearch(), 1000);
    input.onkeydown = (e) => { if (e.key === 'Enter') executeAddDesignSearch(); };
    input.oninput = doSearch;
    creatorInput.onkeydown = (e) => { if (e.key === 'Enter') executeAddDesignSearch(); };
    creatorInput.oninput = doSearch;
}

async function executeAddDesignSearch() {
    const query = document.getElementById('addDesignSearchInput').value.trim();
    if (!query) return;
    const creator = document.getElementById('addDesignCreatorInput').value.trim();
    const statusEl = document.getElementById('addDesignSearchStatus');
    const candidatesEl = document.getElementById('addDesignCandidates');
    statusEl.innerHTML = '<span class="search-spinner"></span> 検索中...';
    statusEl.style.display = 'block';
    candidatesEl.innerHTML = '';
    try {
        const creatorParam = creator ? `&creator=${encodeURIComponent(creator)}` : '';
        const url = `${SHEET_URL}?action=search&type=book&q=${encodeURIComponent(query)}${creatorParam}`;
        const res = await fetch(url);
        const data = await res.json();
        const results = data.results || [];
        if (results.length === 0) {
            statusEl.textContent = '候補が見つかりませんでした。';
            document.getElementById('addDesignManualArea').style.display = 'block';
            return;
        }
        document.getElementById('addDesignManualArea').style.display = 'none';
        statusEl.style.display = 'none';
        candidatesEl.innerHTML = results.map(c =>
            buildCandidateCard(c, `selectAddDesignCandidate(${JSON.stringify(c).replace(/"/g, '&quot;')})`)
        ).join('');
    } catch (err) {
...
function selectAddDesignCandidate(candidate) {
    selectedDesignCandidate = candidate;
    document.getElementById('addDesignStep1').style.display = 'none';
    document.getElementById('addDesignStep2').style.display = 'block';
    const el = document.getElementById('addDesignSelectedInfo');
    if (el) el.innerHTML = buildSelectedInfo(candidate);
}

function proceedAddDesignManual() {
    selectedDesignCandidate = null;
    const title = document.getElementById('addDesignSearchInput').value.trim();
    const creator = document.getElementById('addDesignCreatorInput').value.trim();
    document.getElementById('addDesignStep1').style.display = 'none';
    document.getElementById('addDesignStep2').style.display = 'block';
    const el = document.getElementById('addDesignSelectedInfo');
    if (el) el.innerHTML = `<div class="selected-info-text"><p style="font-weight:bold;">${title}</p>${creator ? `<p style="color:#666;font-size:0.9rem;">${creator}</p>` : ''}</div>`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.onclick = () => {
            ...
            else if (targetTab === 'design') renderDesignBooks();
    ...
    // デザイン本：検索・フィルター
    document.getElementById('searchBoxDesign')?.addEventListener('input', renderDesignBooks);
    document.getElementById('designCategoryFilter')?.addEventListener('change', renderDesignBooks);

    // デザイン本：直接追加
    document.getElementById('openAddDesignBtn')?.addEventListener('click', openAddDesignModal);
    document.getElementById('addDesignClose')?.addEventListener('click', () => document.getElementById('addDesignModal').classList.remove('active'));
    document.getElementById('addDesignSearchBtn')?.addEventListener('click', executeAddDesignSearch);
    document.getElementById('addDesignManualBtn')?.addEventListener('click', proceedAddDesignManual);
    document.getElementById('addDesignResearchBtn')?.addEventListener('click', () => {
        selectedDesignCandidate = null;
        document.getElementById('addDesignStep1').style.display = 'block';
        document.getElementById('addDesignStep2').style.display = 'none';
        document.getElementById('addDesignCandidates').innerHTML = '';
    ...
    document.getElementById('submitAddDesignBtn')?.addEventListener('click', async () => {
        const title = selectedDesignCandidate ? selectedDesignCandidate.title : document.getElementById('addDesignSearchInput').value.trim();
        if (!title) return alert('タイトルを入力・選択してください');
        const data = {
            action:   'addDesignBook',
            title,
            author:   selectedDesignCandidate ? selectedDesignCandidate.creator : document.getElementById('addDesignCreatorInput').value.trim(),
            category: document.getElementById('addDesignCategory').value,
            memo:     document.getElementById('addDesignMemo').value,
            coverUrl: document.getElementById('addDesignCoverUrl').value.trim() || (selectedDesignCandidate?.coverUrl || ''),
            externalId: selectedDesignCandidate?.externalId || ''
        };
        const btn = document.getElementById('submitAddDesignBtn');
        btn.textContent = '保存中...'; btn.disabled = true;
        await sendToGAS(data, btn, 'デザイン本棚に追加する');
    });

    // デザイン本：Wishlistから昇格
    document.getElementById('designFromWishClose')?.addEventListener('click', () => document.getElementById('designFromWishModal').classList.remove('active'));
    document.getElementById('submitDesignFromWishBtn')?.addEventListener('click', async () => {
        const data = {
            action:   'promoteWishToDesign',
            title:    designFromWishPayload.title,
            author:   designFromWishPayload.creator || '',
            coverUrl: designFromWishPayload.coverUrl || '',
            externalId: designFromWishPayload.externalId || '',
            category: document.getElementById('designFromWishCategory').value,
            memo:     document.getElementById('designFromWishMemo').value
        };
        const btn = document.getElementById('submitDesignFromWishBtn');
        btn.textContent = '保存中...'; btn.disabled = true;
        await sendToGAS(data, btn, 'デザイン本棚に追加する');
    });
